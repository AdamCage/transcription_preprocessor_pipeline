...
description: >-
  ...
  (coarse seg, Silero VAD ONNX, chunking, OpenAI STT via openai/httpx, Gemma chat ASR
  via Ollama/vLLM, merge) and scripts/eval_test_audio.py (XLSX, stereo-call, tqdm).
  Use when the user works on this workspace, ASR preprocessing, ina, VAD, Gemma-4,
  eval reports, or Windows TF/uv extras.
---
...
- **`audio_asr_pipeline/config.py`** — `PipelineConfig`, `VLLMTranscribeConfig` (`stt_backend`, `api_key`, `base_url`, `model`, `gemma_api_style`, `gemma_asr_prompt`, `gemma_max_tokens`), `STTBackendName` (`httpx`/`openai`/`gemma`), `VADBackendName` (`silero`/`none`), `GemmaApiStyle` (`ollama_native`/`openai_chat`).
- **`audio_asr_pipeline/pipeline.py`** — `AudioTranscriptionPipeline`: shared **`_file_sem`** from `max_concurrent_files`; CPU prep in `run_in_executor`; then async chunk STT. Triple backend via `create_stt_client`.
...
- **`audio_asr_pipeline/preprocess.py`** — load → coarse → VAD (Silero or `none`) → speech `TimeSpan[]`.
...
- **`scripts/eval_test_audio.py`** — `--stereo-call`, `--coarse-backend`, `--ina-allow-gpu`, `--stt-backend` (`httpx`/`openai`/`gemma`), `--api-key`, `--concurrency`, `--no-vad`, `--gemma-api-style`, tqdm progress.
...
7. **Skip VAD for fast debugging** — `--no-vad` sets `vad_backend="none"`; audio chunked by duration only.
